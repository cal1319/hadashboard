...
@newFile1 = "assets/images/NorCal/new.jpg"
@oldFile1 = "assets/images/NorCal/old.jpg"

# Change "OHX" in the file << open... line to your radar station ID. Check README for link.
def fetch_image(old_file,new_file)
	`rm #{old_file}` 
	`mv #{new_file} #{old_file}`	
	open('assets/images/NorCal/new.jpg', 'wb') do |file|
...
SCHEDULER.every @fetchNewImageEvery, first_in: 0 do
	...
	send_event('NorCal', image: make_web_friendly(@oldFile1))
	sleep(@cameraDelay)
	send_event('NorCal', image: make_web_friendly(new_file1))
end
